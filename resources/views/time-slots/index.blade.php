                    ...
                    <div class="card border shadow-xs mb-4">
                        <div class="card-header border-bottom pb-0">
                            <div class="d-sm-flex align-items-center justify-content-between">
                                <div>
                                    <h6 class="font-weight-semibold text-lg mb-0">Gestion des Créneaux Horaires</h6>
                                    <p class="text-sm">Liste et configuration des créneaux disponibles</p>
                                </div>
                                <div class="ms-auto d-flex align-items-center">
                                    ...
                                    <a href="{{ route('time-slots.create') }}" class="btn btn-sm btn-dark btn-icon d-flex align-items-center">
                                        <span class="btn-inner--icon">
                                            <svg width="16" height="16" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="d-block me-2">
                                                <path d="M6.25 6.375a4.125 4.125 0 118.25 0 4.125 4.125 0 01-8.25 0zM3.25 19.125a7.125 7.125 0 0114.25 0v.003l-.001.119a.75.75 0 01-.363.63 13.067 13.067 0 01-6.761 1.873c-2.472 0-4.786-.684-6.76-1.873a.75.75 0 01-.364-.63l-.001-.122zM19.75 7.5a.75.75 0 00-1.5 0v2.25H16a.75.75 0 000 1.5h2.25v2.25a.75.75 0 001.5 0v-2.25H22a.75.75 0 000-1.5h-2.25V7.5z" />
                                            </svg>
                                        </span>
                                        <span class="btn-inner--text">Ajouter un Créneau</span>
                                    </a>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card-body px-0 py-0">
                            @if($timeSlots->isEmpty())
                                <div class="text-center py-2">
                                    <p class="font-weight-semibold text-lg mb-0">Aucun créneau horaire n'a été créé</p>
                                </div>
                            @else
                                    ...
                                    <table class="table align-items-center mb-0 table-bordered text-center">
                                        <thead class="bg-gray-100 text-center">
                                            <tr>
                                                ...
                                                <th class="text-secondary text-xs font-weight-semibold opacity-7">Heure de Début</th>
                                                <th class="text-secondary text-xs font-weight-semibold opacity-7">Heure de Fin</th>
                                                <th class="text-secondary text-xs font-weight-semibold opacity-7">Statut</th>
                                        ...
                                        <tbody>
                                            @foreach ($timeSlots as $timeSlot)
                                                <tr>
                                                    <td class="text-sm font-weight-semibold">{{ $timeSlot->id }}</td>
                                                    <td class="text-sm font-weight-semibold">{{ $timeSlot->start_time }}</td>
                                                    <td class="text-sm font-weight-semibold">{{ $timeSlot->end_time }}</td>
                                                    <td class="align-middle">
                                                        <span class="badge {{ $timeSlot->is_active ? 'bg-success' : 'bg-secondary' }}">
                                                            {{ $timeSlot->is_active ? 'Actif' : 'Inactif' }}
                                                        </span>
                                                    </td>
                                                    <td class="align-middle text-center">
                                                        <div class="d-flex justify-content-center gap-2">
                                                            <a href="{{ route('time-slots.edit', $timeSlot->id) }}" class="btn btn-link text-warning px-3 mb-0">
                                                            ...
                                                            <form action="{{ route('time-slots.destroy', $timeSlot->id) }}" method="POST" class="d-inline">
                                                                @csrf
                                                                @method('DELETE')
                                                                <button type="submit" class="btn btn-link text-danger px-3 mb-0" onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce créneau ?')">
                                ...
                                <div class="border-top py-3 px-3 d-flex align-items-center">
                                    ...
                                    @else
                                        <a href="{{ $timeSlots->previousPageUrl() }}" class="btn btn-sm btn-white d-sm-block d-none mb-0">Précédent</a>
                                    @endif

                                    <nav aria-label="Pagination" class="ms-auto">
                                        <ul class="pagination pagination-light mb-0">
                                            @foreach ($timeSlots->links()->elements[0] as $page => $url)
                                    ...
                                    @if ($timeSlots->hasMorePages())
                                        <a href="{{ $timeSlots->nextPageUrl() }}" class="btn btn-sm btn-white d-sm-block d-none mb-0 ms-auto">Suivant</a>
                                    @else
